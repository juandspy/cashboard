...
def accounts_to_account_names(list_x): return [
    x.name for x in list_x]  # just a test helper


def accounts_to_account_parents(list_x): return [
    x.parent for x in list_x]  # just a test helper
...
def test_get_account_children():
    ...
    assert accounts_to_account_parents(sub_accounts) == [
        CURRENT_ASSETS_NAME, CURRENT_ASSETS_NAME]
    ...
    assert accounts_to_account_parents(sub_accounts) == [
        BANK_ACCOUNT_NAME, BANK_ACCOUNT_NAME, CURRENT_ASSETS_NAME]
...
def test_get_book_assets():
    ...
    assert accounts_to_account_names(
        assets) == [BANK_ACCOUNT_NAME, CASH_ACCOUNT_NAME]
    assert accounts_to_account_parents(
        assets) == [CURRENT_ASSETS_NAME, CURRENT_ASSETS_NAME]

    assets = get_book_accounts_of_type('ASSET', mock_store.book, depth=3)
    assert accounts_to_account_names(
        assets) == [BANK_SUB_ACCOUNT_1_NAME, BANK_SUB_ACCOUNT_2_NAME, CASH_ACCOUNT_NAME]
    assert accounts_to_account_parents(
        assets) == [BANK_ACCOUNT_NAME, BANK_ACCOUNT_NAME, CURRENT_ASSETS_NAME]
