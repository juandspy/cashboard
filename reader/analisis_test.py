...
def test_get_daily_delta():
    ...
    assert got_df[pd.Timestamp(opening_transaction_date.date())] == BANK_INITIAL_VALUE
    assert got_df[pd.Timestamp(transaction_date.date())] == -TRANSACTION_VALUE
...
def test_get_daily_balance():
    ...
    assert got_df[pd.Timestamp(opening_transaction_date.date())] == BANK_INITIAL_VALUE
    assert (
        got_df[pd.Timestamp(transaction_date.date())]
